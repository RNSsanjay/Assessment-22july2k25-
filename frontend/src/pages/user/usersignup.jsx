...
const UserSignup = () => {
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        setFormMessage('Sign up successful! Redirecting...');
        login(data.token, data.user);
      } else {
        setFormMessage(data.error || 'Sign up failed. Please try again.');
      }
    } catch (error) {
      setFormMessage('Sign up failed. Please try again.');
  ...
  return (
    <div className="min-h-screen flex font-inter">
      ...
      <div
        ...
        style={{ backgroundImage: "url('/src/assets/signup_img.svg')", position: 'relative' }}
        onClick={() => navigate('/user/login')}
      >
        <div className="absolute inset-0 bg-black opacity-50 group-hover:opacity-60 transition-all duration-300"></div>
      ...
      <div className="w-full md:w-3/5 bg-[#F5F5F7] p-8 md:p-12 flex flex-col justify-center">
        <div className="max-w-md mx-auto w-full">
          ...
          <form className="space-y-6" onSubmit={handleSignUp}>
            <div>
              <label htmlFor="name" className="block text-[#6C6C6C] text-sm font-bold mb-2 uppercase tracking-wide">Your Name</label>
              <input
                type="text"
                id="name"
            ...
            <div>
              <label htmlFor="password" className="block text-[#6C6C6C] text-sm font-bold mb-2 uppercase tracking-wide">Password</label>
              <input
                type="password"
                id="password"
            ...
            <div>
              <label htmlFor="confirmPassword" className="block text-[#6C6C6C] text-sm font-bold mb-2 uppercase tracking-wide">Confirm Password</label>
              <input
                type="password"
                id="confirmPassword"
